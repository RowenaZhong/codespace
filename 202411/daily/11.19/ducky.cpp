...
{
    if(restN==0&&restLen==0)return 1;
    if(restN==0||restLen==0)return 0;
    int ans=0;
    for(int i=0;i<=d*place;i++)
    {
        if(i*len>restLen||i>restN)return 0;
        ans=(ans+dfs(len+1,place*d-i,restLen-i*len,restN-i))%mod;
    }
    return ans;
}
int main()
